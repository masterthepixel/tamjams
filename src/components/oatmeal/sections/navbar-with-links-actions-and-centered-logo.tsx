...
export function NavbarLink({
  ...
  return (
    <Link
      href={href}
      className={clsx(
        "group inline-flex items-center justify-between gap-2 text-3xl/10 font-medium text-olive-950 lg:text-sm/7 dark:text-white transition-colors duration-200",
        className
      )}
      {...props}
    >
      {children}
      <span
        className="inline-flex p-1.5 opacity-0 group-hover:opacity-100 lg:hidden text-current"
        aria-hidden="true"
      >
        <svg
          fill="none"
...
export function NavbarWithLinksActionsAndCenteredLogo({
  ...
  return (
    <header
      className={clsx(
        "sticky top-0 z-50 bg-olive-100 dark:bg-olive-950 border-b border-olive-200 dark:border-olive-800",
        className
      )}
      {...props}
    >
      ...
      <nav>
        ...
        <ElDialog className="lg:hidden">
          <dialog id="mobile-menu" className="backdrop:bg-transparent text-olive-950 dark:text-white">
            <ElDialogPanel className="fixed inset-0 bg-olive-100 px-6 py-6 lg:px-10 dark:bg-olive-950 overflow-y-auto">
